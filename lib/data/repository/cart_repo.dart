...
class CartRepo extends GetxController {
  ...
  void setCartList(List<CartModel> cartList) {
    ...
    cartList.forEach((element) {
      ...
      return cart.add(jsonEncode(
          element)); // json 형태로 객체를 바꾸어서 리스트에 저장한다. 왜냐면 SharedPreferences 는 오직 String 만 받으니깐....
    });
    shardPreferences.setStringList(AppConstants.CART_LIST,
        cart); // Cart-list 라는 이름으로 SharedPreferences 안에 List 를 통째로 저장한다
    print(
  ...
  List<CartModel> getCartList() {
    ...
    carts.forEach((element) {
      ...
      cartList.add(CartModel.fromJson(jsonDecode(
          element))); // 딱 한가지를 놓쳤다. 무조건 스트링으로 되어 있는걸 jsonDecode 를 통해서 Map 으로 바꾸었고 그걸 다시 fromJson 으로 객체로 바꾸었다.
    });
