...
class PopularProductController extends GetxController {
  ...
  void initProduct(ProductModel product, CartController cart) {
    // 매번 페이지에 들어갈 때 마다 새롭게 _quantity를 초기화 한다는 거지..
  ...
  void addItem(ProductModel product) {
    if (_quantity > 0) {
      ...
      Get.snackbar(
          "Thank you for purchasing", "We added your item(s) in the Cart.",
          backgroundColor: AppColors.mainColor, colorText: Colors.blue);
      _quantity =
          0; // _quantity 를 0 으로 해주는 이유는 추가되었는데 만약 또 추가를 한다면 거기에 대한 추가 값이 들어가야 하기 때문이다. 안해주면 그 디테일 화면에서 계속 추가가 된다. 7 더했는데 1개 들려 8이 되었는데 맵에는 15가 더해지기 때문이지.
