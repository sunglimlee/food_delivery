...
class PlaceOrderModel {
  ...
  List<CartModel> get cart => _cart!; // 이렇게 getter 도 만들었고, maybe setter 도 만들어야 하지 않았을까 싶다.
  ...
  PlaceOrderModel.fromJson(Map<String, dynamic> json) { // fromJson 만들었고
    // 그안에 들어있는 배열을 이렇게 forEach 를 이용해서 객체화해서 변수에 넣는구나.
    if (json['cart'] != null) {
      List<CartModel> cart = []; // 초기화를 시키고
      // 맵안의 각각의 멤버들을 가지고 나눈거고
      json['cart'].forEach((v) { // 당연히 forEach 를 사용해야지.. 왜냐면 이미 객체이니깐.
  ...
  Map<String, dynamic> toJson() { // 당연히 toJson, toList, Map 3개를 한꺼번에 사용해야지.. 그래야지 Json 객체로 넘어가지..
    final Map<String, dynamic> data = <String, dynamic>{}; // 대괄호만 써도 되는데 앞에 형을 붙여줬네..
    if (_cart != null) {
      data['cart'] = _cart!.map((v) => v.toJson()).toList(); // v.toJson 하나를 맵으로 바꾸고 그걸 toList 해서 다시 맵으로 바꾼거다.
    }
